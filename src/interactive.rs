...
pub(crate) unsafe fn interactive_receive_data(
    ...
    if (*mm).b_flags & MODES_ACFLAGS_LLEITHER_VALID != 0 {
        ...
        if ((*mm).b_flags | (*a).b_flags) & MODES_ACFLAGS_LLEITHER_VALID
...
// of Aircraft
//
unsafe fn interactive_create_aircraft(mut mm: *mut ModesMessage) -> *mut Aircraft {
    ...
    if (*mm).msgtype == 32 as c_int {
        ...
        if mode_c < -(12 as c_int) {
            a.mode_ac_flags |= MODEAC_MSG_MODEA_ONLY
        } else {
            (*mm).altitude = mode_c * 100 as c_int;
...
//
unsafe fn interactive_find_aircraft(mode_s: *const ModeS, addr: u32) -> *mut Aircraft {
