...
fn fix_bit_errors(mm: &mut ModesMessage, maxfix: c_int, bit_error_table: &[ErrorInfo]) {
    // NOTE(unwrap): Should not panic as bits should always be <= 122; maxfix <= 2
    let bits = i8::try_from(mm.msgbits).unwrap();
    let maxfix = u8::try_from(maxfix).unwrap();
    let syndrome = mode_s_checksum(mm.msg, mm.msgbits);
    mm.correctedbits = bit_error_table
        .binary_search_by(|e| e.syndrome.cmp(&syndrome))
        .ok()
        .and_then(|index| {
            let ei = &bit_error_table[index];
        ...
        .map(|ei| {
            ...
            let mut i = 0;
            while i < ei.bits {
                bitpos = ei.pos[usize::from(i)] - offset;
            ...
            while i < ei.bits {
                ...
                *fresh1 ^= 1 << 7 - (bitpos & 7);
                let fresh2 = res;
                res = res + 1;
                mm.corrected[usize::from(fresh2)] = bitpos as c_char;
                i += 1
            }
            c_int::from(res)
        })
